export function FeaturedSkeleton({ count = 8 }: { count?: number }) {
  return (
    <div className="space-y-8 animate-pulse">
      {/* Hero skeleton */}
      <div className="h-64 sm:h-80 rounded-3xl bg-muted/60 border border-border" />

      {/* Proof strip skeleton */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {Array.from({ length: 4 }).map((_, i) => (
          // biome-ignore lint/suspicious/noArrayIndexKey: skeleton
          <div key={i} className="h-20 rounded-2xl bg-muted/40 border border-border" />
        ))}
      </div>
